...
class Individual:    
    ...
    def __init__(self, neural_network: MLP, 
                 ...
                 individual_id: int,
                 max_p_connect = 1, max_p_disconnect = 1):
        
        self.max_p_connect = max_p_connect
        self.max_p_disconnect = max_p_disconnect
        ...
        self.p_connect = torch.rand(1) * self.max_p_connect
        self.p_disconnect = torch.rand(1) * self.max_p_disconnect
        ...
        for i in range(len(genome_masks_length)):
            if init_fully_connected:
                ...
                p_pruned = beta_sampler.sample()
                p_mask = torch.rand(genome_masks_length[i])
                mask = p_mask > p_pruned
    ...
    def get_feature_disconnected_units(self, tot_disconnectable_units):
        ...
        for m in self.genome_masks:
            connections = torch.sum(m, axis=1)                
