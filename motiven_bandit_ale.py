...
class MOTIVEN(object):
    def __init__(self):
        ...
        self.max_epochs = 1500  # 700  # Number of epochs to finish the experiment
    ...
    def get_data(self, n_test=100):
        ...
        action_data = [] # save the action at each time step of the trial
        distance_data = [] # get the distance from the robot's effector and the target (if this is too much work do not worry, I think it is redundant)
        goal_data = []
        
        for _ in range(n_test):
            current_goal = self.goal_manager.goal_selector('c0')
            goal_data.append(current_goal)
            # I do not use this line. I do not want parameters to be updated
            # self.goal_manager.current_goal = active_goal
            self.simulator.set_active_goal(current_goal)
            ...
            distance_data_trial = [] # get the distance from the robot's effector and the target (if this is too much work do not worry, I think it is redundant)
            for _ in range(self.max_it_trial):
                pass
                # perform a sequence action and get simulator data
                action = self.method_to_get_action_without_learning() # <- I think this is the only thing I need
                action_data.append(action)
                self.simulator.apply_action(action)
                sim_data.append(self.simulator.get_scenario_data())
                if self.simulator.get_reward():  #the current target ball has been reached
                   break

            sim_data.append(sim_data_trial)
            action_data.append(action_data_trial)
            distance_data.append(distance_data_trial)
            
        return sim_data, action_data, distance_data, goal_data
    ...
    def main(self):
        ...
        c_epoch = self.n_epochs
        
        iter_count = 0
        while self.n_epochs <= self.max_epochs:
            # print(self.it_trial)
            # print(self.goal_manager.q_values)
            ...
            if c_epoch != self.n_epochs:
                self.reward_extrinsic[c_epoch] /= (iter_count - 1)
                print(self.reward_extrinsic[c_epoch])
                iter_count = 0
                if c_epoch % 50 == 0:
                    np.savez(self.folder + "test_data_" + str(self.n_epochs) + ".npz", 
                              action_data = action_data,
                              sensor_simulator = sensor_simulator,
                              sensor_episode_t = sensor_episode_t,
                              sensor_episode_t1 = sensor_episode_t1,
            ...
            action_data.append(action)
            sensor_simulator.append(self.simulator.get_sensorization())
            sensor_episode_t.append(self.episode.get_sensorial_state_t())
            sensor_episode_t1.append(self.episode.get_sensorial_state_t1())
            motivation_type.append(self.candidate_state_evaluator.intrinsic_exploration_type)
            goal_active.append(self.simulator.active_goal)
            goal_state.append(self.simulator.get_goals_state())
            goal_competence.append(self.goal_manager.goals_competence)
            
            # reward.append(self.simulator.get_reward())
            # reward.append(self.episode.get_reward())
            epoch_data.append(self.n_epochs)
            trial_data.append(self.n_trials)
