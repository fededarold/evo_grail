# -*- coding: utf-8 -*-
"""
Created on Wed Nov 22 17:25:07 2023
...
def place_individuals_in_grid(grails):
    ...
    for i in range(len(grails)):
        ...
        E_scaled = InfoDiscrete.range_scaler(grails[i].entropy_goal,
                                             E_lower_bound, E_upper_bound)
        
        x, y = get_individual_xy_grid(feature_x=E_scaled, 
                                      feature_y=I_scaled)
...
if __name__ == "__main__":
    ...
    for i in range(len(grails)):
        ...
        E_values.append(grails[i].entropy_goal)
    I_upper_bound = max(I_values)
    I_lower_bound = min(I_values)
    E_upper_bound = max(E_values)
    E_lower_bound = min(E_values)
    
    I_upper_history.append(I_upper_bound)
    I_lower_history.append(I_lower_bound)
    E_upper_history.append(E_upper_bound)
    E_lower_history.append(E_lower_bound)
    ...
    for c_iteration in range(1,N_ITERATIONS):  
        ...
        np.savez(f_name,  
                  I_upper_history=np.array(I_upper_history),
                  I_lower_history=np.array(I_lower_history),
                  E_upper_history=np.array(E_upper_history),
                  E_lower_history=np.array(E_lower_history),
                  map_reward=map_reward,
                  map_spreading=map_spreading,
                  map_competence=map_competence,
                  # grails=grails_map,
